# Crankshaft Image Builder

Tool used to create Crankshaft images for Raspberry Pi Android Auto head units,
based on the latest pi-gen repository structure from the Raspberry Pi Foundation.

**Note**: This is a refactored version that aligns with the latest pi-gen structure for better maintainability.

**Note**: Crankshaft images are built for both 32-bit and 64-bit ARM architectures.

https://getcrankshaft.com/

## About

Crankshaft is built on top of the official Raspberry Pi OS generation tool (pi-gen) with custom stages and modifications to create an automotive-focused Linux distribution. This refactored version uses the latest pi-gen structure for better compatibility and maintenance.
...
- Custom audio and video pipeline
